...
| `peekWidth` | `85` | Pane width as a percentage of the full safe viewport width, `50`–`100`. At `100`, symmetric gutters leave exactly enough room for the outside button rail. |
| `peekHeight` | `95` | Pane height as a percentage of the full safe viewport height, `50`–`100`. |
...
| `reducedEffects` | off | Drop backdrop and control blur, override `backdropBlur`, and replace source-link morphs with short fades. Worth it on integrated graphics. |
...
src/frame/frame.*               authenticated target wrapper and message relay
...
**A wrapper preserves authenticated embeds.** The target lives one level below
an extension-origin wrapper, making every HTTP(S) target cross-origin from its
parent while keeping the request in a real `chrome-extension://` context. With
host permission for the target, Chromium can treat that request as same-site
for cookies, so Jira, Confluence and similar applications can reuse the
browser's existing session. Neither layer uses `sandbox`; the cross-origin
boundary provides DOM isolation without forcing the target into an opaque
...
src/frame/                    authenticated target wrapper and message relay
